function weekNumberMondayStart(date) {
    ...
    // forces weekNumber to be two digits, ie 07 for 7th week
...
var Dater = function () {
    ...
    var yearWeek = function (str) {
        now = (str !== undefined) ? new Date(Date.parse(str)) : now;
        return "week-" + now.getFullYear() + "-" + weekOfYear();
    }
    var fileName = function (str) {
        now = (str !== undefined) ? new Date(Date.parse(str)) : now;
        return now.getFullYear() + "-" + weekOfYear() + "-" + dayOfWeek();
    }
