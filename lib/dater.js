function getISO8601WeekNumber(date) {
    // Get an int fort the week number for a date, according to ISO 8601
    // with Monday as the start of the week.
...
function fmtWeekISO8601 (weekNumber) {
    if ((weekNumber[0] === 'W') || (weekNumber[0] === 'w')) {
        weekNumber = weekNumber.substr(1);
    }
...
var Dater = function () {
    ...
    var weekOfYear = function (str) {
        now = (str !== undefined) ? new Date(Date.parse(str)) : now;
        return getISO8601WeekNumber(now);
    }

    var getWeekISO = function (str) {
        now = (str !== undefined) ? new Date(Date.parse(str)) : now;
        return fmtWeekISO8601(getISO8601WeekNumber(now));
    }
    ...
    var fileName = function (str) {
        now = (str !== undefined) ? new Date(Date.parse(str)) : now;
        return now.getFullYear() + "-" + fmtWeekISO8601(getISO8601WeekNumber(now)) + "-" + dayOfWeek();
    }

    var fileNameFromDate = function (date) {
        //force date argument
        return now.getFullYear() + "-" + fmtWeekISO8601(getISO8601WeekNumber(date)) + "-" + dayOfWeek();
    }

    var listDayFilesForWeek = function (str) {
        now = (str !== undefined) ? new Date(Date.parse(str)) : now;
        var week = getISO8601WeekNumber(now);
